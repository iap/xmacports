...
# ---- M: gitlab-ci-verify.sh fails when CI has not covered HEAD ----
echo "M gitlab-ci-verify HEAD-coverage gate:"
gv="$DOTFILES_ROOT/.ci/scripts/gitlab-ci-verify.sh"
# The gate must exist and be wired to a commit comparison, not just a status.
grep -q 'CHECK_COVERAGE' "$gv" && grep -q 'HEAD_SHA' "$gv" &&
  grep -q 'no-coverage' "$gv" &&
  {
    ok "coverage check present (--no-coverage opt-out + HEAD compare)"
  } || bad "gitlab-ci-verify.sh missing the HEAD-coverage check"
# Negative control: prove the check can fail. Run against a fake GitLab API
...
EOFAKE
chmod +x "$MT/bin/glab"
# Resolve the real HEAD so the fake is genuinely stale.
...
if [ "$mrc" -ne 0 ] && grep -q 'does not match HEAD' "$MT/out.txt"; then
  ok "stale green pipeline now FAILS with coverage message (rc=$mrc)"
else
