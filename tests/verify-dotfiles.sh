...
test_path=$(HOME="$HOME" \
  ...
  bash -c "mkdir -p \"$HOME/bin\" \"$HOME/.local/bin\"; echo \"DEBUG: HOME=\$HOME, created dirs:\"; ls -ld \"$HOME/bin\" \"$HOME/.local/bin\"; unset DOTFILES_PLATFORM_LOADED; source '$DOTFILES_ROOT/.config/env.d/platform.sh'; echo \"PATH=\$PATH\"" | grep '^PATH=' | cut -d= -f2-)
