...
# Fail-silent, time-bounded fetch (timeout kills a stalled/black-holed SSH session
# so login can never hang). `|| true` swallows failures (offline, auth, timeout).
timeout 10 git -C "$DOTFILES_ROOT" fetch origin --quiet 2>/dev/null || true
