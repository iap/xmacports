...
**Store topology — two age recipients:**

The store is encrypted to **two** recipients so that losing either single key
never makes it unreadable:

- **Primary machine key** — `~/.config/sops/age/keys.txt` (generated by
  `make secrets-init`, never committed).
- **Offline recovery key** — private half stored **off this machine**
  (password manager / encrypted USB); its public half is the second recipient
  in `.sops.yaml`.

Verify both paths after any recipient change:

```bash
sops -d secrets/secrets.enc.yaml > /dev/null && echo "primary: OK"
SOPS_AGE_KEY_FILE=/path/to/recovery-key.txt \
  sops -d secrets/secrets.enc.yaml > /dev/null && echo "recovery: OK"
```

Rotating or adding recipients: edit `.sops.yaml`, then `make secrets-encrypt`
(re-encrypts to the recipients listed there).
...
already-encrypted `secrets/secrets.enc.yaml`). The recipient age public keys are
also in `.sops.yaml` (primary + offline recovery); only holders of a matching
